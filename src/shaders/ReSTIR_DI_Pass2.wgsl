...
fn BRDF(N : vec3<f32>, L : vec3<f32>,V : vec3<f32>, ThreadID : vec3<u32>) -> vec3<f32>
{
    ...
    let BaseColor       : vec3<f32> = textureLoad(G_AlbedoTexture, ThreadID.xy, 0).xyz; // 일단 알베도값으로 넣었습니다
    let Metalness       : f32       = textureLoad(G_EmissiveTexture, ThreadID.xy, 0).w;
    let Roughness       : f32       = textureLoad(G_AlbedoTexture, ThreadID.xy, 0).w;
...
fn BTDF(N : vec3<f32>, L : vec3<f32>,V : vec3<f32>, ThreadID : vec3<u32>) -> vec3<f32>
{
    let Albedo      : vec3<f32> = textureLoad(G_AlbedoTexture, ThreadID.xy, 0).xyz;
    let Roughness   : f32       = textureLoad(G_AlbedoTexture, ThreadID.xy, 0).w;
    ...
    N  = select(-N, N, bViewNormalSameHemisphere);
...
fn BSDF(SampledLight : Light, ThreadID : vec3<u32>) -> vec3<f32>
{
    let L : vec3<f32> = -SampledLight.Direction;  //x2->x1
    let V : vec3<f32> = vec3<f32>(0.0, 0.0, -1.0); //x1->x0 .....근데이게맞는지모르겠습니다
...
fn CalculateP_hat(ThreadID: vec3<u32>, lightSample : Light) -> vec3<f32>
{
    ...
    for (var LightID : u32 = 0u; LightID < UniformBuffer.LightSourceCount; LightID++)
    {
        ...
        let HitPoint : vec3<f32> = textureLoad(G_PositionTexture, ThreadID.xy, 0).xyz;
        let HitNormal : vec3<f32> = text(G_NormalTexture, ThreadID.xy, 0).xyz;

        if (LightSource.LightType == 0u) // Directional Light
        {
            let L   : vec3<f32> = -LightSource.Direction;
            let End : vec3<f32> = HitPoint + L * 1e11;

            BSDFValue           = BSDF(LightSource, ThreadID);
            VisibilityFactor    = Visibility(HitPoint, End);
        ...
        {
            let D : f32         = length(LightSource.Position - HitPoint);
            let L : vec3<f32>   = (LightSource.Position - HitPoint) / D;

            BSDFValue           = BSDF(HitInfo, L, OutDirection);
            VisibilityFactor    = Visibility(HitPoint, LightSource.Position);
        ...
        else if (LightSource.LightType == 2u) // Rect Light
        {
            ...
            let D : f32         = length(SamplePoint - HitPoint);
            let L : vec3<f32>   = (SamplePoint - HitPoint) / D;

            BSDFValue           = BSDF(HitInfo, L, OutDirection);
            VisibilityFactor    = Visibility(HitPoint, SamplePoint);
...
fn UpdateReservoir(LightSampleID: u32, RIS_Weight: f32, Confidence: u32, ThreadID: vec2<u32>){
    ...
    var RandomSeed : f32 = GetHashValue(ThreadID.x * 1342u + ThreadID.y * 4233u + UniformBuffer.FrameIndex * 21337u);
    var SampleID : f32 = textureLoad(ReservoirTexture, ThreadID, 0).x
    var W_sum = textureLoad(ReservoirTexture, ThreadID, 0).y
    let P_Change = RIS_Weight / (RIS_Weight + W_sum);

    if(Random(&RandomSeed) < P_Change){
        SampleID = LightSampleID;
    }
    textureStore(ReservoirTexture, ThreadID, vec4<f32>(SampleID, W_sum+RIS_Weight, 0.0, 0.0));
...
fn cs_main(@builtin(global_invocation_id) ThreadID: vec3<u32>)
{
    ...
    let MIS_Weight : f32 = 1.0 / LIGHT_SAMPLE;
    // 1. M개의 광원 추출
    for (var iter : u32 = 0u; iter < LIGHT_SAMPLE; iter++)
    {
        ...
        let P_hat : vec3<f32> = CalculateP_hat(ThreadID, LightSample);
        let RIS_Weight  : f32 = MIS_Weight * P_hat / P_Light;  //ω_i
        let Confidence  : u32 = 1u;

        UpdateReservoir(SampledLightID, RIS_Weight, Confidence, ThreadID);
    }
